					...
					<div class="m-name">
						...
						<div class="m-buttons fr">
							...
							<el-popover class="u-plansBox" placement="bottom" width="160" v-model="addPlans" trigger="manual">
								...
								<el-button type="success" size="mini" plain class="u-add-to-item-list" @click="openMyPlans" slot="reference">
...
export default {
	...
	methods: {
		...
		openMyPlans() {
			...
			this.addPlans = true;
			let list = this.$store.state.my_item_plans || [];
			list.length
				? (this.myPlansList = list)
				: get_my_item_plans({ limit: 10 }).then((res) => {
						this.$store.commit("SET_STATE", { key: "my_item_plans", value: res.data.data.data });
						this.$set(this.myPlansList, res.data.data.data);
				  });
		},
		// 加入我的清单
		addMyPlans(item) {
			let plan = {
				...
				public: item.public,
				type: item.type,
				relation: JSON.stringify([{ title: item.title, data: [] }]),
			...
			save_item_plan(plan)
				.then((data) => {
					...
					console.log(data, "?");
					if (data.code === 200) {
						this.$message({ message: "加入物品清单成功", type: "success" });
						// setTimeout(() => {
