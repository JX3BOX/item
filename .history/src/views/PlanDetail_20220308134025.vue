			...
			<template slot="body">
				...
				<div class="m-plan-item" v-if="plan.type == '1'">
					<div class="m-item" v-for="(item, index) in plan.relation" :key="index">
						...
						<div class="u-content">
							<div class="u-item" v-for="(el, key) in item.data" :key="key">
								<span class="u-name" :class="`quality-${el.Quality}`">
									<img class="u-icon" :src="iconLink(el.IconID)" />
									<span class="u-count">{{ el.Name }}</span>
								</span>
								<span class="u-name"> {{ el.Name }}</span>
							</div>
						</div>
					</div>
				</div>
