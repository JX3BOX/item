			...
			<template slot="body">
				...
				<div class="m-plan-item" v-if="plan.type == '1'">
					<div class="m-item" v-for="(item, index) in plan.relation" :key="index">
						...
						<div class="u-content">
							<router-link v-for="(el, key) in item.data" :key="key" :to="{ name: 'view', params: { item_id: el.id } }">
								<jx3-item-simple :item="el" />
								<!-- <el-popover placement="bottom-end" width="auto" :visible-arrow="false" trigger="hover">
									<div class="u-item" slot="reference">
										<span class="u-img">
											<img class="u-icon" :src="iconLink(el.IconID)" />
											<span class="u-count">{{ el.count }}</span>
										</span>
										<span class="u-name" :class="`quality-${el.Quality}`"> {{ el.Name }}</span>
									</div>
									<jx3-item :item_id="el.id" />
								</el-popover> -->
							</router-link>
						</div>
					</div>
...
import ItemIcon from "@jx3box/jx3box-editor/src/ItemSimple";
...
export default {
	name: "demo",
	props: [],
	components: { Comment, WikiPanel, "jx3-item-simple": ItemSimple },
	data: function () {
		return {
			loading: false,
