					...
					<div class="m-item" v-for="(item, index) in plan.relation" :key="index">
						...
						<div class="u-content">
							<router-link class="u-item" v-for="(el, key) in item.data" :key="key" :to="{ name: 'view', params: { item_id: el.id } }">
								<el-popover placement="right-end" popper-class="m-simple-item-popup" width="auto" :visible-arrow="false" trigger="hover" transition="none" :close-delay="0" v-model="visible" @show="item_id = item.id"></el-popover>
							</router-link>
							<!-- <div >
								<span class="u-img">
