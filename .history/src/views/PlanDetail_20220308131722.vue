...
export default {
	...
	methods: {
		...
		converted(data) {
			...
			if (_arr.length) this.getConverted([...new Set(_arr)]);
		},
		getConverted(_arr) {
			searchItemsID({ ids: _arr, limit: val.length }).then((res) => {
				let cache_list = res.data.map((item) => {
                ...
                if(this.data.type == 1 ? this.idToItem(val) : this.idToEquip(val))
			});
		},
		// 编辑清单
		editPlan(plan_id) {
